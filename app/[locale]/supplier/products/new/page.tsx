...
import { getCategoryTree, getFilterableAttributes } from "@/lib/queries/catalog";
...
export default async function NewPlantPage({
  ...
  const [categories, attributes] = await Promise.all([getCategoryTree(locale), getFilterableAttributes()]);
  ...
  return (
    <main className="mx-auto max-w-[1280px] px-5 py-8">
      ...
      <AddPlantWizard locale={locale} categories={categories} attributes={attributes} />
    </main>
  );
}
